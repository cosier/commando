...
impl<'c> Tree<'c> {
    ...
    fn monitor(self) -> Self {
        Tree {
            app: self.app.subcommand(SubCommand::with_name("monitor").about(
                "live status monitor",
            )),
        }
    }
    ...
    fn logs(self) -> Self {
        Tree {
            app: self.app.subcommand(
                SubCommand::with_name("logs").about("status").arg(
                    Arg::with_name("debug").short("d").help(
                        "print debug information verbosely",
                    ),
                ),
            ),
        ...
        Tree {
            app: self.app.subcommand(
                SubCommand::with_name("config")
                    ...
                    .subcommand(SubCommand::with_name("compile").about(
                        "Configuration tools",
                    ))
                    .arg(
